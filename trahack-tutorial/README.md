...
- API、DBから非同期通信でデータを取得(fetch)する
- 特定の値が変更したらデータを再取得(refetch)する

### useEffect内で非同期処理通信

useEffectでコンポーネントの生成時にapiを叩き、buttonをクリックするとidをランダムに取得して変更する。そのたびにuseEffectの第2引数にidを指定しているので値が変わるたびにuseEffect内の処理が走る。
